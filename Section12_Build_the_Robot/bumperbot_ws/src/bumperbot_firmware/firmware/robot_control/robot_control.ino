...
// Encoder pulse count (calibrated for gearbox)
const double GEAR_RATIO = 30.0;     // <- Adjust if different (e.g., 50, 90, etc.)
const double PPR = 11.0;            // Pulses per revolution of motor shaft
const double CPR = PPR * GEAR_RATIO; // Counts per output shaft revolution
const double CMD_SCALE = 0.1;
// Encoders
...
void loop() {
  if (Serial.available()) {
    ...
    if(chr == 'r') {
      ...
      if(is_right_wheel_cmd) {
        right_wheel_cmd_vel = atof(value) * CMD_SCALE;
      } else if(is_left_wheel_cmd) {
        left_wheel_cmd_vel = atof(value) * CMD_SCALE;
  ...
  if(current_millis - last_millis >= interval) {
    const double rads_per_rpm = 0.10472;  // 2π / 60
    right_wheel_meas_vel = (10 * right_encoder_counter * (60.0 / CPR)) * rads_per_rpm;
    left_wheel_meas_vel  = (10 * left_encoder_counter  * (60.0 / CPR)) * rads_per_rpm;
